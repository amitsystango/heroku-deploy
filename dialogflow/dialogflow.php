...
putenv('GOOGLE_APPLICATION_CREDENTIALS=Propert-6c0ad00f0fed.json');
$intent_text = (isset($_REQUEST['intent_text']) && $_REQUEST['intent_text'] != '')?$_REQUEST['intent_text']:'';
$session = (isset($_REQUEST['session']) && $_REQUEST['session'] != '')?$_REQUEST['session']:'';
if(trim($intent_text) == '') {
    echo json_encode(array('Error' => 'Intent Text Missing'));
    die;
}

detect_intent_texts('propert-616eb', array($intent_text), $session);
die;
